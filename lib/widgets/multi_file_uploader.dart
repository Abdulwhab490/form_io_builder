      ...
      child: FormField(
              ...
              if (formFieldState.hasError) ...[
                ...
                Container(
                    margin: const EdgeInsets.symmetric(
                        horizontal: 16 * 1.5, vertical: 16 * 0.5),
        ...
        onSaved: (value) {
          widget.map?[widget.Item['key']] = _files
                  .where((e) => e.status == statusUpload.uploaded)
                  .map((es) => es.onlinePath)
                  .toList() ??
              [];
        },
        validator: (value) {
          if (widget.Item['validate'] != null &&
            ...
            if (_files.isEmpty) {
              return widget.Item['validate']['customMessage'] ??
                  'Field is Required';
            }
...
class FileUplaod {
  ...
  FileUplaod(
      {this.file,
      this.status = statusUpload.initi,
      this.onlinePath,
      this.progress = 0})
        ...
        InkWell(
          ...
          child: image!=null?
              Image.file(File(image!),
               ...
               :CachedNetworkImage(
                  imageUrl: fileUplaod.onlinePath!,
